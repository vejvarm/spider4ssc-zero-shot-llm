...
import evaluate
from spider4ssc_zeroshot.vendor.ut5_ssc.seq2seq.metrics.spidercypher.spider_test_suite import compute_test_suite_metric
from spider4ssc_zeroshot.vendor.ut5_ssc.seq2seq.metrics.spidercypher.spider_exact_match import compute_exact_match_metric
import datasets
...
class SpiderCypher(evaluate.Metric):
    def __init__(
        self,
        ...
        **kwargs
        ...
        self.lang: Optional[str] = kwargs.get("metric_lang", "cypher")
        if self.lang not in ["cypher"]: raise NotImplementedError("Metric langugage must be `cypher`")
    ...
    def _compute(self, predictions, references, lang: str):
        ...
        if self.config_name == "test_suite" or self.config_name == "both":
            test_suite = compute_test_suite_metric(predictions, references, db_dir=self.test_suite_db_dir, lang=lang)
        else:
            test_suite = dict()
