...
Spider4SSC can be copied from a local source tree with `spider4ssc-zeroshot prepare-data --source ...`. Remote archive downloads support optional `dataset.archive_sha256` verification; set it for release reproduction so mutable URLs are checked against a recorded digest.
